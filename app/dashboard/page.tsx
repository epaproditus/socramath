...
export default async function DashboardPage() {
  ...
  if (!session || !session.user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-zinc-50 dark:bg-zinc-950 space-y-4">
        ...
        <SignIn />
        <Link href="/" className="text-xs text-zinc-400 hover:text-indigo-500 flex items-center gap-1 mt-4">
          <ArrowLeft className="w-3 h-3" /> Back to Student View
        </Link>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-zinc-50 dark:bg-zinc-950 space-y-4 text-center p-4">
        <div className="bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 p-4 rounded-xl">
          ...
          <p className="text-sm">You are signed in as <strong>{session.user.email}</strong>.</p>
          <p className="text-sm">This account is not authorized to view the Dashboard.</p>
        </div>
        <SignIn />
        <Link href="/" className="text-xs text-zinc-400 hover:text-indigo-500 flex items-center gap-1">
          <ArrowLeft className="w-3 h-3" /> Back to Student View
        </Link>
      </div>
    );
  ...
  return <TeacherDashboard mode="live" />;
}
